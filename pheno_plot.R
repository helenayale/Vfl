...
for (pheno_i in 1:length(pheno_list)){
  ...
  ggsave(paste('box_', pheno, '_DL', '.png', sep = ''), p)
}

# ggplot(data = dat_merge, mapping = aes(x = Treat, y = sos, group = Treat, color = Treat)) + geom_boxplot()
...
# remove outliers eos
out <- boxplot.stats(dat_merge$eos)$out
out
out_ind <- which(dat_merge$eos %in% c(out))
out_ind
...
for (sd_i in 1:length(sd_list)){
  ...
  eval(parse(text = paste('ggsave("box_sd_',sd_n, '_DL.png", p)', sep = '')))
}


# remove outliers sos
out <- boxplot.stats(dat_pix$sd_sos)$out
out
out_ind <- which(dat_pix$sd_sos %in% c(out))
out_ind
...
ggplot(data = dat_out, mapping = aes(x = Treat, y  = sd_sos,  group = Treat, color = Treat)) + geom_boxplot()

# remove outliers eos
out <- boxplot.stats(dat_pix$sd_eos)$out
out
out_ind <- which(dat_pix$sd_eos %in% c(out))
out_ind
...
ggplot(data = dat_out, mapping = aes(x = Treat, y  = sd_eos,  group = Treat, color = Treat)) + geom_boxplot()


# remove outliers los
out <- boxplot.stats(dat_pix$sd_los)$out
out
out_ind <- which(dat_pix$sd_los %in% c(out))
out_ind
...
ggplot(data = dat_out, mapping = aes(x = Treat, y  = sd_los,  group = Treat, color = Treat)) + geom_boxplot()
...
plt <- ggplot(
  ...
  labs(
    title = paste("Boxplot of SOS EOS (Double Logistic)", sep = " "),
    y = "DOY", x = "Year"
  ) +
  annotate(
    "text", x = 2022.8, y = c(120, 300), label = c("SOS", "EOS")
  )+
  scale_x_continuous(
    breaks = c(2017:2022)
  ) +
